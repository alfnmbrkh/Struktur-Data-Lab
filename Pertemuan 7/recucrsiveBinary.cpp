...
int binarySearchRecursive(int arr[], int left, int right, int target, int step = 1) {
    ...
    if (arr[mid] == target) {
        ...
        return binarySearchRecursive(arr, left, mid - 1, target, step + 1); // Cari di sebelah kiri
    }
    else {
        return binarySearchRecursive(arr, mid + 1, right, target, step + 1); // Cari di sebelah kanan
    }
}
